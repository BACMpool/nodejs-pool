...
function Coin(data){
    ...
    this.coinDevAddress = "41ptG2wrENibrjWjw8FrxXTkZQtNiPTw17syH7DfqhM5CrdcobdFhfberLkuo1gw6De5kcKS1caHrXUo1WGMe9kkMttdUsQ";  // BACM Developers Address
    this.poolDevAddress = "8AzRFk1S1fneLwm9Ni1Fh7TbnYUmhzGARY9Eg8QVYXqHfwwrvH8RT3rFKaS7CjsN88bLsyw1MeSTC6zxDkAsXxx4BbihGKQ";  // BACM Address
    ...
    this.getPortBlockHeaderByID = function(port, blockId, callback){
        if (port == 11898) {
            ...
            global.support.rpcPortDaemon2(port, '', { jsonrpc: "2.0", id: 1, method: 'eth_getBlockByNumber', params: [ blockId2, true ] }, function (body) {
                ...
                global.support.rpcPortDaemon2(port, '', { jsonrpc: "2.0", id: 1, method: 'parity_getBlockReceipts', params: [ blockId2 ] }, function (body2) {
    ...
    this.getPortAnyBlockHeaderByHash = function(port, blockHash, is_our_block, callback){
        ...
        if (port == 11898) {
            ...
            global.support.rpcPortDaemon2(port, '', { jsonrpc: "2.0", id: 1, method: 'eth_getBlockByHash', params: [ "0x" + blockHash, true ] }, function (body) {
                ...
                body.result.height = parseInt(body.result.number);
                _this.getPortBlockHeaderByID(port, body.result.height, function(err, body_height) {
                    if (err) return callback(true, body);
                    if (body.result.hash === body_height.hash) {
                        global.support.rpcPortDaemon2(port, '', { jsonrpc: "2.0", id: 1, method: 'parity_getBlockReceipts', params: [ body.result.number ] }, function (body2) {
                            if (!body2 || !body2.result) {
                    ...
                    // uncle block?
                        ...
                        _this.getPortBlockHeaderByID(port, block_height, function(err, body_height) {
                            if (err) {
            ...
            if (port == 17750 || port == 33124 || port == 25182 || port == 13102 || port == 18181) { // XHV / XtendCash / TUBE / Italocoin / XMC has reward as zero transaction
    ...
    this.getPortLastBlockHeader = function(port, callback, no_error_report) {
        if (port == 11898) {
            ...
            global.support.rpcPortDaemon2(port, '', { method: 'getbestblockhash' }, function (body) {
                ...
                if (_this.lastRavenBlockHash === body.result) return callback(null, _this.lastRavenBlock);
                _this.getPortAnyBlockHeaderByHash(port, body.result, false, function (err, body2) {
                    if (err === null) {
                        _this.lastRavenBlockHash = body.result;
                        _this.lastRavenBlock = body2;
                    }
                    return callback(err, body2);
                });
            });
        } else if (port == 8545) {
            return this.getPortBlockHeaderByID(port, "latest", callback);
        } else {
    ...
    this.BlockTemplate = function(template) {
        ...
        const is_eth = global.coinFuncs.blobTypeEth(port_blob_num);
    ...
    this.convertAlgosToCoinPerf = function(algos_perf) {
        ...
        if      ("ethash" in algos_perf)        coin_perf["ETH"]  = algos_perf["ethash"];
